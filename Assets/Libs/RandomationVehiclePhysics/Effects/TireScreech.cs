...
namespace RVP {
    ...
    public class TireScreech : MonoBehaviour {
        ...
        private void Start() {
            ...
            for (var i = 0; i < vp.wheels.Length; i++) {
                wheels[i] = vp.wheels[i];
                if (vp.wheels[i].GetComponent<TireMarkCreate>()) {
                    var newThreshold = vp.wheels[i].GetComponent<TireMarkCreate>().slipThreshold;
        ...
        private void Update() {
            ...
            for (var i = 0; i < vp.wheels.Length; i++) {
                if (wheels[i].connected) {
                    if (Mathf.Abs(F.MaxAbs(wheels[i].sidewaysSlip, wheels[i].forwardSlip, alwaysScrape)) -
                        slipThreshold > 0) {
                        if (wheels[i].popped)
                            nonePopped = false;
                        else
                    ...
                    if (wheels[i].grounded && GroundSurfaceMaster.surfaceTypesStatic != null && GroundSurfaceMaster.surfaceTypesStatic.Length > 0) {
                    ...
                    screechAmount = Mathf.Max(screechAmount,
                        Mathf.Pow(
                            Mathf.Clamp01(
                                Mathf.Abs(F.MaxAbs(wheels[i].sidewaysSlip, wheels[i].forwardSlip, alwaysScrape)) -
                                slipThreshold), 2));
                }
            }
            ...
            if (screechAmount > 0) {
                if (!snd.isPlaying) {
                    ...
                    snd.volume = Mathf.Lerp(snd.volume,
                        screechAmount * (vp.groundedWheels * 1.0f / (wheels.Length * 1.0f)), 2 * Time.deltaTime);
