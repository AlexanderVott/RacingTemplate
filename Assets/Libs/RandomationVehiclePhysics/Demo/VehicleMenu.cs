...
namespace RVP {
    ...
    public class VehicleMenu : MonoBehaviour {
        ...
        public void SpawnVehicle(int vehicle) {
            newVehicle = Instantiate(vehicles[vehicle], spawnPoint,
                             Quaternion.LookRotation(spawnRot, GlobalControl.worldUpDir)) as GameObject;
            ...
            var trans = newVehicle.GetComponentInChildren<Transmission>();
            if (trans) {
            ...
            if (newVehicle.GetComponent<FlipControl>() && newVehicle.GetComponent<StuntDetect>()) {
                newVehicle.GetComponent<FlipControl>().flipPower =
                    stuntToggle.isOn && assistToggle.isOn ? new Vector3(10, 10, -10) : Vector3.zero;
        ...
        public void SpawnChaseVehicle() {
            if (chaseCarSpawnTime == 0) {
                chaseCarSpawnTime = 1;
                var chaseCar = Instantiate(chaseVehicle, spawnPoint,
                                   Quaternion.LookRotation(spawnRot, GlobalControl.worldUpDir)) as GameObject;
        ...
        public void SpawnChaseVehicleDamage() {
            if (chaseCarSpawnTime == 0) {
                chaseCarSpawnTime = 1;
                var chaseCar = Instantiate(chaseVehicleDamage, spawnPoint,
                                   Quaternion.LookRotation(spawnRot, GlobalControl.worldUpDir)) as GameObject;
                chaseCar.GetComponent<FollowAI>().target = newVehicle.transform;
            }
        }
