...
namespace RVP {
    ...
    public class Suspension : MonoBehaviour {
        ...
        [System.NonSerialized] public bool flippedSide;
        [System.NonSerialized] public float flippedSideFactor;
        ...
        [System.NonSerialized] public float steerDegrees;
        ...
        [System.NonSerialized] public float camberAngle;
        ...
        [Tooltip("Angle at which the suspension points out to the side")] [Range(-89.999f, 89.999f)]
        ...
        [System.NonSerialized] public List<SuspensionPart> movingParts = new List<SuspensionPart>();

        [Header("Spring")] public float suspensionDistance;
        [System.NonSerialized] public float compression;

        [Tooltip("Should be left at 1 unless testing suspension travel")] [Range(0, 1)]
        public float targetCompression;
        [System.NonSerialized] public float penetration; // How deep the ground is interesecting with the wheel's tire
        ...
        [System.NonSerialized] public Vector3 springDirection;
        [System.NonSerialized] public Vector3 upDir;      // Local up direction
        [System.NonSerialized] public Vector3 forwardDir; // Local forward direction

        [System.NonSerialized] public DriveForce targetDrive; // The drive being passed into the wheel

        [System.NonSerialized] public SuspensionPropertyToggle properties; // Property toggler
        [System.NonSerialized] public bool steerEnabled = true;
        [System.NonSerialized] public bool steerInverted;
        [System.NonSerialized] public bool driveEnabled = true;
        [System.NonSerialized] public bool driveInverted;
        [System.NonSerialized] public bool ebrakeEnabled = true;
        [System.NonSerialized] public bool skidSteerBrake;

        [Header("Damage")] [Tooltip("Point around which the suspension pivots when damaged")]
        public Vector3 damagePivot;

        [Tooltip("Compression amount to remain at when wheel is detached")] [Range(0, 1)]
        public float detachedCompression = 0.5f;

        public float jamForce = Mathf.Infinity;
        [System.NonSerialized] public bool jammed;
            ...
            if (Application.isPlaying) {
                ...
                if (generateHardCollider) {
                    ...
                    compressCol.height =
                        (wheel.popped
                             ? wheel.rimRadius
                             : Mathf.Lerp(wheel.rimRadius, wheel.tireRadius, wheel.tirePressure)) * 2;
        ...
        private void FixedUpdate() {
            ...
            if (wheel.connected) {
                compression = Mathf.Min(targetCompression,
                    suspensionDistance > 0 ? Mathf.Clamp01(wheel.contactPoint.distance / suspensionDistance) : 0);
            ...
            if (wheel.grounded && wheel.connected) {
                ...
                if (suspensionDistance > 0 && targetCompression > 0) {
                    var appliedSuspensionForce =
                        (leaningForce
                             ? Vector3.Lerp(upDir, vp.norm.forward,
                                 Mathf.Abs(Mathf.Pow(Vector3.Dot(vp.norm.forward, vp.upDir), 5)))
                             : vp.norm.forward) *
                        springForce *
                        (Mathf.Pow(springForceCurve.Evaluate(1 - compression), Mathf.Max(1, springExponent)) -
                ...
                if (compression == 0 && !generateHardCollider && applyHardContactForce)
                    rb.AddForceAtPosition(
                        -vp.norm.TransformDirection(0, 0,
                            Mathf.Clamp(travelVel, -hardContactSensitivity * TimeMaster.fixedTimeFactor, 0) +
        ...
        private void GetSpringVectors() {
            ...
            var casterDir = -Mathf.Sin(casterAngle * Mathf.Deg2Rad) * flippedSideFactor;
            var sideDir = -Mathf.Sin(sideAngle * Mathf.Deg2Rad);

            springDirection =
                tr.TransformDirection(casterDir, Mathf.Max(Mathf.Abs(casterDir), Mathf.Abs(sideDir)) - 1, sideDir)
                    .normalized;
        }
        ...
        private void GetCamber() {
            if (solidAxleCamber && oppositeWheel && wheel.connected) {
                if (oppositeWheel.wheel.rim && wheel.rim) {
                    var axleDir =
                        tr.InverseTransformDirection((oppositeWheel.wheel.rim.position - wheel.rim.position)
                            .normalized);
                ...
                camberAngle =
                    camberCurve.Evaluate(
                        Application.isPlaying && wheel.connected ? wheel.travelDist : targetCompression) + camberOffset;
            }
        }
        ...
        public void UpdateProperties() {
            if (properties)
                foreach (var curProperty in properties.properties) {
        ...
        private void OnDrawGizmosSelected() {
            ...
            if (wheel)
                if (wheel.rim) {
                    var wheelPoint = wheel.rim.position;

                    var camberSin = -Mathf.Sin(camberAngle * Mathf.Deg2Rad);
                    var steerSin = Mathf.Sin(Mathf.Lerp(steerRangeMin, steerRangeMax, (steerAngle + 1) * 0.5f) *
                                             Mathf.Deg2Rad);
                    var minSteerSin = Mathf.Sin(steerRangeMin * Mathf.Deg2Rad);
                    var maxSteerSin = Mathf.Sin(steerRangeMax * Mathf.Deg2Rad);
