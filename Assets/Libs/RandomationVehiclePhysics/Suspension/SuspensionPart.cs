...
namespace RVP {
    ...
    public class SuspensionPart : MonoBehaviour {
        ...
        [Header("Connections")] [Tooltip("Object to point at")]
        public Transform connectObj;

        [Tooltip("Local space point to point at in connectObj")]
        public Vector3 connectPoint;
        [System.NonSerialized] public Vector3 initialConnectPoint;
        private Vector3 localConnectPoint; // Transformed connect point
        ...
        private Vector3 parentUpDir; // Parent's up direction
            ...
            if (tr) {
                if (!solidAxle && (suspension && !solidAxleConnector || solidAxleConnector)) {
                    ...
                    if (isHub && wheel && !solidAxleConnector) {
                        if (wheel.rim) {
                            ...
                            tr.localEulerAngles = new Vector3(tr.localEulerAngles.x, tr.localEulerAngles.y,
                                -suspension.casterAngle * suspension.flippedSideFactor);
                        }
                    }
                        ...
                        if (rotate) {
                            tr.rotation = Quaternion.LookRotation((localConnectPoint - tr.position).normalized,
                                solidAxleConnector ? tr.parent.forward : suspension.upDir);
                        ...
                        if (stretch && Application.isPlaying)
                            tr.localScale = new Vector3(tr.localScale.x, tr.localScale.y,
                                initialScale.z * (Vector3.Distance(tr.position, localConnectPoint) / initialDist));
                    }
                }
                    ...
                    if (wheel1.rim && wheel2.rim && wheel1.suspensionParent && wheel2.suspensionParent) {
                        ...
                        tr.rotation =
                            Quaternion.LookRotation(((wheelConnect1 + wheelConnect2) * 0.5f - tr.position).normalized,
                                parentUpDir);
                        tr.localEulerAngles = new Vector3(
                            tr.localEulerAngles.x,
                            tr.localEulerAngles.y,
                            Vector3.Angle((wheelConnect1 - wheelConnect2).normalized, tr.parent.right) *
                            Mathf.Sign(Vector3.Dot((wheelConnect1 - wheelConnect2).normalized, parentUpDir)) *
                            Mathf.Sign(tr.localPosition.z) * (invertRotation ? -1 : 1));
                    }
                }
            }
        }
