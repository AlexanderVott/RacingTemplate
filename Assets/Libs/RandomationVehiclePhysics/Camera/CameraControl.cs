    ...
    public class CameraControl : MonoBehaviour {
        ...
        private void FixedUpdate() {
            if (target && targetBody && target.gameObject.activeSelf) {
                ...
                lookDir = Vector3.Slerp(lookDir,
                    xInput == 0 && yInput == 0 ? Vector3.forward : new Vector3(xInput, 0, yInput).normalized,
                    0.1f * TimeMaster.inverseFixedTimeFactor);
                smoothYRot = Mathf.Lerp(smoothYRot, targetBody.angularVelocity.y,
                ...
                if (Physics.Raycast(target.position, -targetUp, out hit, 1, castMask) && !stayFlat)
                    upLook = Vector3.Lerp(upLook, Vector3.Dot(hit.normal, targetUp) > 0.5 ? hit.normal : targetUp,
                ...
                var lookDirActual = (lookDir - new Vector3(Mathf.Sin(smoothYRot), 0, Mathf.Cos(smoothYRot)) *
                                     Mathf.Abs(smoothYRot) * 0.2f).normalized;
                var forwardDir = lookObj.TransformDirection(lookDirActual);
                var localOffset = lookObj.TransformPoint(-lookDirActual * distance -
                                                         lookDirActual *
                                                         Mathf.Min(targetBody.velocity.magnitude * 0.05f, 2) +
                                                         Vector3.up * height);
