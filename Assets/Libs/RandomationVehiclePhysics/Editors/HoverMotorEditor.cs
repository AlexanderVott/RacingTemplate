...
namespace RVP {
    ...
    public class HoverMotorEditor : Editor {
        ...
        public override void OnInspectorGUI() {
            ...
            var targetScript = (HoverMotor) target;
            var allTargets = new HoverMotor[targets.Length];
            ...
            if (targetScript.wheels != null) {
                if (targetScript.wheels.Length == 0)
                    ...
                    EditorGUILayout.LabelField(
                        "Top Speed (Estimate): " + (topSpeed * 2.23694f).ToString("0.00") + " mph || " +
                        (topSpeed * 3.6f).ToString("0.00") + " km/h", EditorStyles.boldLabel);
            }
            else {
                EditorGUILayout.HelpBox("No wheels are assigned.", MessageType.Warning);
            }
                ...
                if (showButtons)
                    if (GUILayout.Button("Get Wheels"))
                        foreach (var curTarget in allTargets)
                            curTarget.wheels = curTarget.transform.GetTopmostParentComponent<VehicleParent>().transform
