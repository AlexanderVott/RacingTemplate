...
namespace RVP {
    ...
    public class VehicleAssist : MonoBehaviour {
        ...
        [Header("Roll Over")] [Tooltip("Automatically roll over when rolled over")]
        public bool autoRollOver;

        [Tooltip("Roll over with steer input")]
        public bool steerRollOver;

        [System.NonSerialized] public bool rolledOver;
        ...
        [Header("Air")] [Tooltip("Increase angular drag immediately after jumping")]
        ...
        private void FixedUpdate() {
            if (vp.groundedWheels > 0) {
                groundedFactor = basedOnWheelsGrounded
                                     ? vp.groundedWheels / (vp.hover ? vp.hoverWheels.Length : vp.wheels.Length)
                                     : 1;
        ...
        private void ApplySpinAssist() {
            ...
            if (autoSteerDrift) {
                ...
                targetDriftAngle = (steerSign != Mathf.Sign(vp.localVelocity.x) ? vp.steerInput : steerSign) *
                                   -maxDriftAngle;
                var velDir = new Vector3(vp.localVelocity.x, 0, vp.localVelocity.z).normalized;
                var targetDir = new Vector3(Mathf.Sin(targetDriftAngle * Mathf.Deg2Rad), 0,
                    Mathf.Cos(targetDriftAngle * Mathf.Deg2Rad)).normalized;
                var driftTorqueTemp = velDir - targetDir;
                targetTurnSpeed =
                    driftTorqueTemp.magnitude * Mathf.Sign(driftTorqueTemp.z) * steerSign * driftSpinSpeed -
                    vp.localAngularVel.y * Mathf.Clamp01(Vector3.Dot(velDir, targetDir)) * 2;
            }
            else {
                targetTurnSpeed = vp.steerInput * driftSpinSpeed *
                                  (vp.localVelocity.z < 0
                                       ? vp.accelAxisIsBrake ? Mathf.Sign(vp.accelInput) :
                                                               Mathf.Sign(F.MaxAbs(vp.accelInput, -vp.brakeInput))
                                       : 1);
            }

            rb.AddRelativeTorque(
                new Vector3(0,
                    (targetTurnSpeed - vp.localAngularVel.y) * driftSpinAssist *
                    driftSpinCurve.Evaluate(Mathf.Abs(Mathf.Pow(vp.localVelocity.x, driftSpinExponent))) *
                    groundedFactor, 0),
                ForceMode.Acceleration);

            var rightVelDot = Vector3.Dot(tr.right, rb.velocity.normalized);
        ...
        private void ApplyDownforce() {
            if (vp.groundedWheels > 0 || applyDownforceInAir) {
                rb.AddRelativeForce(
                    new Vector3(0,
                        downforceCurve.Evaluate(Mathf.Abs(vp.localVelocity.z)) * -downforce *
                        (applyDownforceInAir ? 1 : groundedFactor) *
                        (invertDownforceInReverse ? Mathf.Sign(vp.localVelocity.z) : 1), 0),
                    ForceMode.Acceleration);
                ...
                if (invertDownforceInReverse && vp.localVelocity.z < 0)
                    rb.AddRelativeTorque(
                        new Vector3(
                            downforceCurve.Evaluate(Mathf.Abs(vp.localVelocity.z)) * downforce *
                            (applyDownforceInAir ? 1 : groundedFactor), 0, 0),
                        ForceMode.Acceleration);
            }
        }
        ...
        private void RollOver() {
            ...
            if (vp.groundedWheels == 0 && vp.velMag < rollSpeedThreshold && vp.upDot < 0.8 && rollCheckDistance > 0) {
                if (Physics.Raycast(tr.position, vp.upDir, out rollHit, rollCheckDistance,
                        GlobalControl.groundMaskStatic)
                    || Physics.Raycast(tr.position, vp.rightDir, out rollHit, rollCheckDistance,
                        GlobalControl.groundMaskStatic)
                    || Physics.Raycast(tr.position, -vp.rightDir, out rollHit, rollCheckDistance,
                        GlobalControl.groundMaskStatic))
                    rolledOver = true;
        ...
        private void ApplyDriftPush() {
            var pushFactor = (vp.accelAxisIsBrake ? vp.accelInput : vp.accelInput - vp.brakeInput) *
                             Mathf.Abs(vp.localVelocity.x) * driftPush * groundedFactor *
                             (1 - Mathf.Abs(Vector3.Dot(vp.forwardDir, rb.velocity.normalized)));

            rb.AddForce(
                vp.norm.TransformDirection(new Vector3(Mathf.Abs(pushFactor) * Mathf.Sign(vp.localVelocity.x),
