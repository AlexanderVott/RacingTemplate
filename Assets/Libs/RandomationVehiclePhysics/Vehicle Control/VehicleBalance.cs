...
namespace RVP {
    ...
    public class VehicleBalance : MonoBehaviour {
        ...
        private void FixedUpdate() {
            // Apply endo limit
            actualPitchInput = vp.wheels.Length == 1
                                   ? 0
                                   : Mathf.Clamp(vp.pitchInput, -1, vp.velMag > endoSpeedThreshold ? 0 : 1);
        ...
        private void ApplyLean() {
            if (vp.groundedWheels > 0) {
                var inverseWorldUp = vp.norm.InverseTransformDirection(
                    Vector3.Dot(vp.wheelNormalAverage, GlobalControl.worldUpDir) <= 0
                        ? vp.wheelNormalAverage
                        : Vector3.Lerp(GlobalControl.worldUpDir, vp.wheelNormalAverage,
                            Mathf.Abs(Vector3.Dot(vp.norm.up, GlobalControl.worldUpDir)) * 2));
                ...
                targetLean = new Vector3(
                    Mathf.Lerp(
                        inverseWorldUp.x,
                        Mathf.Clamp(
                            -vp.rollInput * leanFactor.z * leanRollCurve.Evaluate(Mathf.Abs(vp.localVelocity.z)) +
                            Mathf.Clamp(vp.localVelocity.x * slideLeanFactor, -leanFactor.z * slideLeanFactor,
                                leanFactor.z * slideLeanFactor), -leanFactor.z, leanFactor.z),
                    ...
                    inverseWorldUp.z *
                    (1 - Mathf.Abs(F.MaxAbs(actualPitchInput * leanFactor.x, vp.rollInput * leanFactor.z))));
            }
            else {
                targetLean = vp.upDir;
            }
            ...
            targetLeanActual = Vector3.Lerp(targetLeanActual, vp.norm.TransformDirection(targetLean),
                (1 - leanSmoothness) * Time.timeScale * TimeMaster.inverseFixedTimeFactor).normalized;
            Debug.DrawRay(tr.position, targetLeanActual, Color.black);

            // Apply pitch
            rb.AddTorque(
                vp.norm.right * -(Vector3.Dot(vp.forwardDir, targetLeanActual) * 20 - vp.localAngularVel.x) * 100 *
                (vp.wheels.Length == 1 ? 1 : leanPitchCurve.Evaluate(Mathf.Abs(actualPitchInput))),
            ...
            rb.AddTorque(
                ...
                (vp.groundedWheels == 1
                     ...
                     : 0) * 100 * leanYawCurve.Evaluate(Mathf.Abs(vp.steerInput)),
                ForceMode.Acceleration);
