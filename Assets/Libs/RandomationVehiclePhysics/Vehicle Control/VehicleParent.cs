...
namespace RVP {
    ...
    public class VehicleParent : MonoBehaviour {
        ...
        [System.NonSerialized] public Transform norm; // Normal orientation object
        ...
        [System.NonSerialized] public float ebrakeInput;
        [System.NonSerialized] public bool boostButton;
        [System.NonSerialized] public bool upshiftPressed;
        [System.NonSerialized] public bool downshiftPressed;
        [System.NonSerialized] public float upshiftHold;
        [System.NonSerialized] public float downshiftHold;
        [System.NonSerialized] public float pitchInput;
        [System.NonSerialized] public float yawInput;
        [System.NonSerialized] public float rollInput;
        ...
        [System.NonSerialized] public float burnout;
        ...
        [System.NonSerialized] public Vector3 localVelocity; // Local space velocity
        [System.NonSerialized] public Vector3 localAngularVel; // Local space angular velocity
        [System.NonSerialized] public Vector3 forwardDir; // Forward direction
        [System.NonSerialized] public Vector3 rightDir; // Right direction
        [System.NonSerialized] public Vector3 upDir; // Up direction
        [System.NonSerialized] public float forwardDot; // Dot product between forwardDir and GlobalControl.worldUpDir
        [System.NonSerialized] public float rightDot; // Dot product between rightDir and GlobalControl.worldUpDir
        [System.NonSerialized] public float upDot; // Dot product between upDir and GlobalControl.worldUpDir
        [System.NonSerialized] public float velMag; // Velocity magnitude
        [System.NonSerialized] public float sqrVelMag; // Velocity squared magnitude
        ...
        private bool wheelLoopDone = false;
        public bool hover;
        [System.NonSerialized] public int groundedWheels;         // Number of wheels grounded
        [System.NonSerialized] public Vector3 wheelNormalAverage; // Average normal of the wheel contact points
        private Vector3 wheelContactsVelocity;                    // Average velocity of wheel contact points
        ...
        [System.NonSerialized] public VehicleParent inputInherit; // Vehicle which to inherit input from

        [System.NonSerialized] public bool crashing;

        [Header("Crashing")] public bool canCrash = true;
        public AudioSource crashSnd;
        public AudioClip[] crashClips;
        [System.NonSerialized] public bool playCrashSounds = true;
        public ParticleSystem sparks;
        [System.NonSerialized] public bool playCrashSparks = true;
        ...
        private void Start() {
            ...
            var normTemp = new GameObject(tr.name + "'s Normal Orientation");
            ...
            if (towVehicle) {
                ...
                newTow.transform.position =
                    tr.TransformPoint(
                        newTow.GetComponent<Joint>().connectedAnchor - newTow.GetComponent<Joint>().anchor);
        ...
        private void FixedUpdate() {
            ...
            norm.transform.rotation =
                Quaternion.LookRotation(groundedWheels == 0 ? upDir : wheelNormalAverage, forwardDir);
            ...
            if (groundedWheels > 0 && !hover && !accelAxisIsBrake && burnoutThreshold >= 0 &&
                ...
                burnout = Mathf.Lerp(burnout,
                    (5 - Mathf.Min(5, Mathf.Abs(localVelocity.z))) / 5 * Mathf.Abs(accelInput),
        ...
        // Set brake input
        public void SetBrake(float f) => brakeInput = accelAxisIsBrake ? -Mathf.Clamp(accelInput, -1, 0) : Mathf.Clamp(f, -1, 1);

        // Set steer input
        public void SetSteer(float f) => steerInput = Mathf.Clamp(f, -1, 1);

        // Set ebrake input
        public void SetEbrake(float f) {
            if ((f > 0 || ebrakeInput > 0) && holdEbrakePark && velMag < 1 && accelInput == 0 &&
                (brakeInput == 0 || !brakeIsReverse))
                ebrakeInput = 1;
            else
        ...
        // Set boost input
        public void SetBoost(bool b) => boostButton = b;

        // Set pitch rotate input
        public void SetPitch(float f) => pitchInput = Mathf.Clamp(f, -1, 1);

        // Set yaw rotate input
        public void SetYaw(float f) => yawInput = Mathf.Clamp(f, -1, 1);

        // Set roll rotate input
        public void SetRoll(float f) => rollInput = Mathf.Clamp(f, -1, 1);

        // Do upshift input
        public void PressUpshift() => upshiftPressed = true;

        // Do downshift input
        public void PressDownshift() => downshiftPressed = true;

        // Set held upshift input
        public void SetUpshift(float f) => upshiftHold = f;

        // Set held downshift input
        public void SetDownshift(float f) => downshiftHold = f;
        ...
        private void SetCenterOfMass() {
            ...
            if (suspensionCenterOfMass) {
                if (hover)
                    for (var i = 0; i < hoverWheels.Length; i++)
                        susAverage = i == 0
                                         ? hoverWheels[i].hoverDistance
                                         : (susAverage + hoverWheels[i].hoverDistance) * 0.5f;
                else
                    for (var i = 0; i < wheels.Length; i++) {
                        var newSusDist = wheels[i].transform.parent.GetComponent<Suspension>().suspensionDistance;
            ...
            rb.inertiaTensor =
                rb.inertiaTensor; // This is required due to decoupling of inertia tensor from center of mass in Unity 5.3
        }
                ...
                for (var i = 0; i < wheels.Length; i++) {
                    if (wheels[i].grounded) {
                        wheelContactsVelocity =
                            i == 0
                                ? wheels[i].contactVelocity
                                : (wheelContactsVelocity + wheels[i].contactVelocity) * 0.5f;
                        wheelNormalAverage =
                            i == 0
                                ? wheels[i].contactPoint.normal
                                : (wheelNormalAverage + wheels[i].contactPoint.normal).normalized;
                    }
        ...
        private IEnumerator WheelCheckLoop() {
            for (var i = 0; i < wheelGroups.Length; i++) {
    ...
    public class WheelCheckGroup {
        ...
        public void Activate() {
            foreach (var curWheel in wheels)
                curWheel.getContact = true;

            foreach (var curHover in hoverWheels)
