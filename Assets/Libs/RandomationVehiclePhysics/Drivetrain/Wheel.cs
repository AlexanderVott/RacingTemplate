...
namespace RVP {
    ...
    public class Wheel : MonoBehaviour {
        ...
        [System.NonSerialized] public VehicleParent vp;
        [System.NonSerialized] public Suspension suspensionParent;
        [System.NonSerialized] public Transform rim;
        ...
        [Header("Rotation")] [Tooltip("Bias for feedback RPM lerp between target RPM and raw RPM")] [Range(0, 1)]
        ...
        [Tooltip("How much the suspension compression affects the wheel friction")] [Range(0, 1)]
        ...
        [System.NonSerialized] public float setTireWidth;
        [System.NonSerialized] public float tireWidthPrev;
        [System.NonSerialized] public float setTireRadius;
        [System.NonSerialized] public float tireRadiusPrev;

        [System.NonSerialized] public float setRimWidth;
        [System.NonSerialized] public float rimWidthPrev;
        [System.NonSerialized] public float setRimRadius;
        [System.NonSerialized] public float rimRadiusPrev;

        [System.NonSerialized] public float actualRadius;

        [Header("Tire")] [Range(0, 1)] public float tirePressure = 1;
        [System.NonSerialized] public float setTirePressure;
        [System.NonSerialized] public float tirePressurePrev;
        private float initialTirePressure;
        public bool popped;
        [System.NonSerialized] public bool setPopped;
        [System.NonSerialized] public bool poppedPrev;
        public bool canPop;

        [Tooltip("Requires deform shader")] public float deformAmount;
        private Material rimMat;
        private Material tireMat;
        private float airLeakTime = -1;

        [Range(0, 1)] public float rimGlow;
        private float glowAmount;
        private Color glowColor;

        [System.NonSerialized] public bool updatedSize;
        [System.NonSerialized] public bool updatedPopped;

        private float currentRPM;
        [System.NonSerialized] public DriveForce targetDrive;
        [System.NonSerialized] public float rawRPM; // RPM based purely on velocity
        [System.NonSerialized] public WheelContact contactPoint = new WheelContact();
        ...
        [System.NonSerialized] public float travelDist;
        private Vector3 upDir; // Up direction
        private float circumference;

        [System.NonSerialized] public Vector3 contactVelocity; // Velocity of contact point
        private float actualEbrake;
        private float actualTargetRPM;
        private float actualTorque;

        [System.NonSerialized] public Vector3 forceApplicationPoint; // Point at which friction forces are applied
        ...
        private void Start() {
            ...
            if (tr.childCount > 0) {
                ...
                if (rimGlow > 0 && Application.isPlaying) {
                    var rimMesh = rim.GetComponent<MeshRenderer>();
                    if (rimMesh != null) {
                        rimMat = new Material(rimMesh.sharedMaterial);
                ...
                if (rim.childCount > 0) {
                    ...
                    if (canDetach) {
                        ...
                        detachTireRend.sharedMaterial =
                            tireMat ? tireMat : tire.GetComponent<MeshRenderer>().sharedMaterial;
                    }
                }
        ...
        private void FixedUpdate() {
            ...
            actualTorque = suspensionParent.driveEnabled
                               ? Mathf.Lerp(targetDrive.torque, Mathf.Abs(vp.accelInput), vp.burnout)
                               : 0;
            ...
            travelDist = suspensionParent.compression < travelDist || grounded
                             ...
                             : Mathf.Lerp(travelDist, suspensionParent.compression,
                                 suspensionParent.extendSpeed * Time.fixedDeltaTime);
            ...
            if (connected) {
                ...
                if (generateHardCollider) {
                    ...
                    else if (tireWidthPrev != setTireWidth || tireRadiusPrev != setTireRadius ||
                ...
                // Burnout spinning
                if (vp.burnout > 0 && targetDrive.rpm != 0 && actualEbrake * vp.ebrakeInput == 0 && connected &&
                    grounded)
                ...
                if (airLeakTime >= 0) {
                    ...
                    if (grounded) {
                        airLeakTime += Mathf.Max(Mathf.Abs(currentRPM) * 0.001f, localVel.magnitude * 0.1f) *
            ...
            if (!Application.isPlaying) {
                ...
                if (deformAmount > 0 && tireMat && connected)
                    if (tireMat.HasProperty("_DeformNormal")) {
                        ...
                        var deformNormal = grounded
                                               ? contactPoint.normal *
                                                 Mathf.Max(
                                                     -suspensionParent.penetration *
                                                     (1 - suspensionParent.compression) * 10, 1 - tirePressure) *
                                                 deformAmount
                                               : Vector3.zero;
                        tireMat.SetVector("_DeformNormal",
                ...
                if (rimMat)
                    if (rimMat.HasProperty("_EmissionColor")) {
                        // Make the rim glow
                        var targetGlow =
                            connected && GroundSurfaceMaster.surfaceTypesStatic[contactPoint.surfaceType].leaveSparks
                                ? Mathf.Abs(F.MaxAbs(forwardSlip, sidewaysSlip))
                                : 0;
                        glowAmount = popped
                                         ? Mathf.Lerp(glowAmount, targetGlow,
                                             (targetGlow > glowAmount ? 2 : 0.2f) * Time.deltaTime)
                                         : 0;
                        glowColor = new Color(glowAmount, glowAmount * 0.5f, 0);
                        rimMat.SetColor("_EmissionColor",
                            popped ? Color.Lerp(Color.black, glowColor, glowAmount * rimGlow) : Color.black);
                    }
            }
        }
        ...
        private void GetWheelContact() {
            var castDist =
                Mathf.Max(
                    suspensionParent.suspensionDistance * Mathf.Max(0.001f, suspensionParent.targetCompression) +
                    actualRadius, 0.001f);
            var wheelHits = Physics.RaycastAll(suspensionParent.maxCompressPoint, suspensionParent.springDirection,
                castDist, GlobalControl.wheelCastMaskStatic);
            RaycastHit hit;
            var hitIndex = 0;
            var validHit = false;
            var hitDist = Mathf.Infinity;

            if (connected) // Loop through raycast hits to find closest one
            ...
            if (validHit) {
                ...
                if (!grounded && impactSnd && (tireHitClips.Length > 0 && !popped || rimHitClip && popped)) {
                    impactSnd.PlayOneShot(
                        popped ? rimHitClip : tireHitClips[Mathf.RoundToInt(Random.Range(0, tireHitClips.Length - 1))],
        ...
        private void GetRawRPM() {
            rawRPM = grounded
                         ? contactPoint.relativeVelocity.x / circumference * (Mathf.PI * 100) *
                           -suspensionParent.flippedSideFactor
                         : Mathf.Lerp(rawRPM, actualTargetRPM,
                             (actualTorque + suspensionParent.brakeForce * vp.brakeInput +
                              actualEbrake * vp.ebrakeInput) * Time.timeScale);
        }
        ...
        private void ApplyFriction() {
            if (grounded) {
                var forwardSlipFactor = (int) slipDependence == 0 || (int) slipDependence == 1
                                            ? forwardSlip - sidewaysSlip
                                            : forwardSlip;
                var sidewaysSlipFactor = (int) slipDependence == 0 || (int) slipDependence == 2
                                             ? sidewaysSlip - forwardSlip
                                             : sidewaysSlip;
                var forwardSlipDependenceFactor =
                    Mathf.Clamp01(forwardSlipDependence - Mathf.Clamp01(Mathf.Abs(sidewaysSlip)));
                var sidewaysSlipDependenceFactor =
                    Mathf.Clamp01(sidewaysSlipDependence - Mathf.Clamp01(Mathf.Abs(forwardSlip)));

                var targetForceX = forwardFrictionCurve.Evaluate(Mathf.Abs(forwardSlipFactor)) *
                ...
                var targetForceZ = sidewaysFrictionCurve.Evaluate(Mathf.Abs(sidewaysSlipFactor)) *
                                   -System.Math.Sign(sidewaysSlip) * (popped ? sidewaysRimFriction : sidewaysFriction) *
                                   sidewaysSlipDependenceFactor *
                                   normalFrictionCurve.Evaluate(Mathf.Clamp01(Vector3.Dot(contactPoint.normal,
                                       GlobalControl.worldUpDir))) *
                                   (vp.burnout > 0 && Mathf.Abs(targetDrive.rpm) != 0 &&
                                    actualEbrake * vp.ebrakeInput == 0 && grounded
                                        ? (1 - vp.burnout) * (1 - Mathf.Abs(vp.accelInput))
                                        : 1);

                var targetForce = tr.TransformDirection(targetForceX, 0, targetForceZ);
                var targetForceMultiplier = (1 - compressionFrictionFactor + (1 - suspensionParent.compression) *
                                             compressionFrictionFactor *
                                             Mathf.Clamp01(
                                                 Mathf.Abs(suspensionParent.tr.InverseTransformDirection(localVel).z) *
                                                 10)) * contactPoint.surfaceFriction;
                frictionForce = Vector3.Lerp(frictionForce, targetForce * targetForceMultiplier,
                ...
                if (contactPoint.col.attachedRigidbody)
                    contactPoint.col.attachedRigidbody.AddForceAtPosition(-frictionForce, contactPoint.point,
                        vp.wheelForceMode);
            }
        }
        ...
        private void ApplyDrive() {
            float brakeForce = 0;
            var brakeCheckValue = suspensionParent.skidSteerBrake ? vp.localAngularVel.y : vp.localVelocity.z;
            ...
            if (!suspensionParent.jammed && connected) {
                var validTorque =
                    !(Mathf.Approximately(actualTorque, 0) && Mathf.Abs(actualTargetRPM) < 0.01f) &&
        ...
        private void PositionWheel() {
            if (suspensionParent)
                rim.position = suspensionParent.maxCompressPoint + suspensionParent.springDirection *
                               suspensionParent.suspensionDistance *
                               ...
                               Mathf.Pow(
                                   Mathf.Max(Mathf.Abs(Mathf.Sin(suspensionParent.sideAngle * Mathf.Deg2Rad)),
                                       Mathf.Abs(Mathf.Sin(suspensionParent.casterAngle * Mathf.Deg2Rad))), 2) *
                               actualRadius +
                               suspensionParent.pivotOffset * suspensionParent.tr.TransformDirection(
                                   Mathf.Sin(tr.localEulerAngles.y * Mathf.Deg2Rad), 0,
                                   Mathf.Cos(tr.localEulerAngles.y * Mathf.Deg2Rad))
                               - suspensionParent.pivotOffset *
        ...
        private void RotateWheel() {
            if (tr && suspensionParent) {
                var ackermannVal = Mathf.Sign(suspensionParent.steerAngle) == suspensionParent.flippedSideFactor
                                       ? 1 + suspensionParent.ackermannFactor
                                       : 1 - suspensionParent.ackermannFactor;
                tr.localEulerAngles = new Vector3(
                    suspensionParent.camberAngle + suspensionParent.casterAngle * suspensionParent.steerAngle *
                    suspensionParent.flippedSideFactor,
                    -suspensionParent.toeAngle * suspensionParent.flippedSideFactor +
                    suspensionParent.steerDegrees * ackermannVal,
                    0);
            }
                ...
                if (checker) {
                    ...
                    if (tireMesh && rimMesh) {
                        ...
                        foreach (var curVert in rimMesh.vertices) {
                            if (new Vector2(curVert.x * scaler.localScale.x, curVert.y * scaler.localScale.y)
                                    .magnitude > maxRadius)
                                maxRadius = new Vector2(curVert.x * scaler.localScale.x,
                                    curVert.y * scaler.localScale.y).magnitude;
        ...
        private void OnDrawGizmosSelected() {
            ...
            var tireActualRadius = Mathf.Lerp(rimRadius, tireRadius, tirePressure);
