...
namespace RVP {
    ...
    public class GasMotor : Motor {
        ...
        [Range(0, 0.99f)] [Tooltip("How quickly the engine adjusts its RPMs")]
        public float inertia;

        [Tooltip("Can the engine turn backwards?")]
        public bool canReverse;
        private DriveForce targetDrive;
        [System.NonSerialized] public float maxRPM;
        ...
        [System.NonSerialized] public bool shifting;
        ...
        public override void FixedUpdate() {
            ...
            actualAccel =
                Mathf.Lerp(vp.brakeIsReverse && vp.reversing && vp.accelInput <= 0 ? vp.brakeInput : vp.accelInput,
                    Mathf.Max(vp.accelInput, vp.burnout), vp.burnout);
            var accelGet = canReverse ? actualAccel : Mathf.Clamp01(actualAccel);
            actualInput = inputCurve.Evaluate(Mathf.Abs(accelGet)) * Mathf.Sign(accelGet);
            targetDrive.curve = torqueCurve;

            if (ignition) {
                var boostEval = boostPowerCurve.Evaluate(Mathf.Abs(vp.localVelocity.z));
                // Set RPM
                targetDrive.rpm = Mathf.Lerp(targetDrive.rpm,
                    actualInput * maxRPM * 1000 * (boosting ? 1 + boostEval : 1), (1 - inertia) * Time.timeScale);
                // Set torque
                if (targetDrive.feedbackRPM > targetDrive.rpm)
                    targetDrive.torque = 0;
                else
                    targetDrive.torque =
                        torqueCurve.Evaluate(targetDrive.feedbackRPM * 0.001f - (boosting ? boostEval : 0)) *
                        Mathf.Lerp(targetDrive.torque, power * Mathf.Abs(System.Math.Sign(actualInput)),
                            (1 - inertia) * Time.timeScale) * (boosting ? 1 + boostEval : 1) * health;

                // Send RPM and torque through drivetrain
                if (outputDrives.Length > 0) {
                    var torqueFactor = Mathf.Pow(1f / outputDrives.Length, driveDividePower);
                    float tempRPM = 0;

                    foreach (var curOutput in outputDrives) {
        ...
        public override void Update() {
            // Set audio pitch
            if (snd && ignition) {
                airPitch = vp.groundedWheels > 0 || actualAccel != 0
                               ? 1
                               : Mathf.Lerp(airPitch, 0, 0.5f * Time.deltaTime);
                pitchFactor = (actualAccel != 0 || vp.groundedWheels == 0 ? 1 : 0.5f) * (shifting
                                          ? pitchIncreaseBetweenShift
                                                ?
                                                Mathf.Sin(transmission.shiftTime / transmission.shiftDelay *
                                                          Mathf.PI)
                                                :
                                                Mathf.Min(transmission.shiftDelay,
