...
namespace RVP {
    ...
    public class GearboxTransmission : Transmission {
        ...
        [System.NonSerialized] public int currentGear;
        private int firstGear;
        [System.NonSerialized] public float curGearRatio; // Ratio of the current gear
        ...
        [System.NonSerialized] public float shiftTime;
        ...
        private void FixedUpdate() {
            ...
            var actualFeedbackRPM = targetDrive.feedbackRPM / Mathf.Abs(curGearRatio);
            var upGearOffset = 1;
            ...
            if (curGearRatio == 0 || shiftTime > 0) {
                ...
                newDrive.rpm =
                    (automatic && skidSteerDrive
                         ? Mathf.Abs(targetDrive.rpm) *
                           Mathf.Sign(vp.accelInput - (vp.brakeIsReverse ? vp.brakeInput * (1 - vp.burnout) : 0))
                         : targetDrive.rpm) / curGearRatio;
            ...
            if (automatic && shiftTime == 0 && vp.groundedWheels > 0) {
                if (!skidSteerDrive && vp.burnout == 0) {
                    if (Mathf.Abs(vp.localVelocity.z) > 1 || vp.accelInput > 0 ||
                        vp.brakeInput > 0 && vp.brakeIsReverse) {
                        if (currentGear < gears.Length - 1
                            && (upperGear.minRPM +
                                upshiftDifference * (curGearRatio < 0 ? Mathf.Min(1, shiftThreshold) : shiftThreshold) -
                                actualFeedbackRPM <= 0 || curGearRatio <= 0 && upperGear.ratio > 0 &&
                                (!vp.reversing || vp.accelInput > 0 && vp.localVelocity.z > curGearRatio * 10))
                            && !(vp.brakeInput > 0 && vp.brakeIsReverse && upperGear.ratio >= 0)
                            && !(vp.localVelocity.z < 0 && vp.accelInput == 0))
                            Shift(1);
                        else if (currentGear > 0
                                 && (actualFeedbackRPM - (lowerGear.maxRPM - downshiftDifference * shiftThreshold) <=
                                     0 || curGearRatio >= 0 && lowerGear.ratio < 0 &&
                                     (vp.reversing || (vp.accelInput < 0 || vp.brakeInput > 0 && vp.brakeIsReverse) &&
                                      vp.localVelocity.z < curGearRatio * 10))
        ...
        public void Shift(int dir) {
            if (health > 0) {
                ...
                while ((skipNeutral || automatic) && gears[Mathf.Clamp(currentGear, 0, gears.Length - 1)].ratio == 0 &&
                       currentGear != 0 && currentGear != gears.Length - 1)
                    currentGear += dir;
        ...
        public void CalculateRpmRanges() {
            ...
            if (!Application.isPlaying) {
                var engine = transform.GetTopmostParentComponent<VehicleParent>().GetComponentInChildren<GasMotor>();
                ...
                for (var i = 0; i < gears.Length; i++) {
                    ...
                    if (gears[i].ratio < 0) {
                        ...
                        if (nextGearRatio == 0)
                            ...
                            gears[i].maxRPM = actualMaxRPM / nextGearRatio +
                                              (actualMaxRPM / nextGearRatio - gears[i].minRPM) * 0.5f;
                    }
                        ...
                        if (prevGearRatio == 0)
                            ...
                            gears[i].minRPM = actualMaxRPM / prevGearRatio -
                                              (gears[i].maxRPM - actualMaxRPM / prevGearRatio) * 0.5f;
                    }
                    else {
                        gears[i].minRPM = 0;
                        gears[i].maxRPM = 0;
