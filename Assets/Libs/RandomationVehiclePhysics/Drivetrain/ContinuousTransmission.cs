...
namespace RVP {
    ...
    public class ContinuousTransmission : Transmission {
        [Tooltip("Lerp value between min ratio and max ratio")] [Range(0, 1)]
        ...
        private void FixedUpdate() {
            ...
            if (health > 0)
                targetRatio = automatic switch {
                    true when vp.groundedWheels > 0 => (1 - vp.burnout) *
                                                       Mathf.Clamp01(
                                                           Mathf.Abs(targetDrive.feedbackRPM) / Mathf.Max(0.01f,
                                                               maxRPM * Mathf.Abs(currentRatio))),
                    false => Mathf.Clamp01(targetRatio +
                                           (vp.upshiftHold - vp.downshiftHold) * manualShiftRate * Time.deltaTime),
                    _ => targetRatio
                };

            reversing = canReverse && vp.burnout == 0 && vp.localVelocity.z < 1 &&
                        (vp.accelInput < 0 || vp.brakeIsReverse && vp.brakeInput > 0);
