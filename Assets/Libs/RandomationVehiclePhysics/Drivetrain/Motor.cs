...
namespace RVP {
    ...
    public abstract class Motor : MonoBehaviour {
        ...
        [System.NonSerialized] public float targetPitch;
        ...
        public virtual void Start() {
            ...
            // Get boost sound
            if (boostLoopSnd) {
                var newBoost = Instantiate(boostLoopSnd.gameObject, boostLoopSnd.transform.position,
                                   boostLoopSnd.transform.rotation) as GameObject;
        ...
        public virtual void FixedUpdate() {
            ...
            // Boost logic
            boost = Mathf.Clamp(
                boosting ? boost - boostBurnRate * Time.timeScale * 0.05f * TimeMaster.inverseFixedTimeFactor : boost,
                0, maxBoost);
            boostPrev = boosting;

            if (canBoost && ignition && health > 0 && !vp.crashing && boost > 0 &&
                (vp.hover
                     ? vp.accelInput != 0 || Mathf.Abs(vp.localVelocity.z) > 1
            ...
            if (boostLoopSnd && boostSnd) {
                switch (boosting) {
                    case true when !boostLoopSnd.isPlaying:
                        boostLoopSnd.Play();
                        break;
                    case false when boostLoopSnd.isPlaying:
                        boostLoopSnd.Stop();
                        break;
                }

                switch (boosting) {
                    case true when !boostPrev:
                        boostSnd.clip = boostStart;
                        boostSnd.Play();
                        break;
                    case false when boostPrev:
        ...
        public virtual void Update() {
            ...
            if (snd) {
                if (ignition && health > 0) {
                    snd.enabled = true;
                    snd.pitch =
                        Mathf.Lerp(snd.pitch, Mathf.Lerp(minPitch, maxPitch, targetPitch), 20 * Time.deltaTime) +
                        Mathf.Sin(Time.time * 200 * (1 - health)) * (1 - health) * 0.1f * damagePitchWiggle;
            ...
            if (boostParticles.Length > 0)
                foreach (var curBoost in boostParticles) {
            ...
            if (smoke) {
                ...
                em.rateOverTime =
                    new ParticleSystem.MinMaxCurve(health < 0.7f ? initialSmokeEmission * (1 - health) : 0);
            }
        }
    }
}