    ...
    public class FlipControl : MonoBehaviour {
        ...
        private void ApplyFlip() {
            ...
            if (freeSpinFlip)
                ...
                flipTorque = new Vector3(
                    ...
                    System.Math.Sign(vp.pitchInput * Mathf.Sign(flipPower.x)) != System.Math.Sign(vp.localAngularVel.x)
                        ? -vp.localAngularVel.x * Mathf.Abs(flipPower.x)
                        : vp.pitchInput * flipPower.x - vp.localAngularVel.x * (1 - Mathf.Abs(vp.pitchInput)) *
                          Mathf.Abs(flipPower.x),
                    vp.yawInput != 0 && Mathf.Abs(vp.localAngularVel.y) > 1 &&
                    System.Math.Sign(vp.yawInput * Mathf.Sign(flipPower.y)) != System.Math.Sign(vp.localAngularVel.y)
                        ? -vp.localAngularVel.y * Mathf.Abs(flipPower.y)
                        : vp.yawInput * flipPower.y -
                          vp.localAngularVel.y * (1 - Mathf.Abs(vp.yawInput)) * Mathf.Abs(flipPower.y),
                    vp.rollInput != 0 && Mathf.Abs(vp.localAngularVel.z) > 1 &&
                    System.Math.Sign(vp.rollInput * Mathf.Sign(flipPower.z)) != System.Math.Sign(vp.localAngularVel.z)
                        ? -vp.localAngularVel.z * Mathf.Abs(flipPower.z)
        ...
        private void ApplyStopFlip() {
            ...
            stopFlipFactor.x = vp.pitchInput * flipPower.x == 0
                                   ? Mathf.Pow(Mathf.Clamp01(vp.upDot),
                                         Mathf.Clamp(10 - Mathf.Abs(vp.localAngularVel.x), 2, 10)) * 10
                                   : 0;
            stopFlipFactor.y = vp.yawInput * flipPower.y == 0 && vp.sqrVelMag > 5
                                   ? Mathf.Pow(Mathf.Clamp01(Vector3.Dot(vp.forwardDir, velDir * Vector3.up)),
                                         Mathf.Clamp(10 - Mathf.Abs(vp.localAngularVel.y), 2, 10)) * 10
                                   : 0;
            stopFlipFactor.z = vp.rollInput * flipPower.z == 0
                                   ? Mathf.Pow(Mathf.Clamp01(vp.upDot),
                                         Mathf.Clamp(10 - Mathf.Abs(vp.localAngularVel.z), 2, 10)) * 10
                                   : 0;

            rb.AddRelativeTorque(
                new Vector3(-vp.localAngularVel.x * stopFlipFactor.x, -vp.localAngularVel.y * stopFlipFactor.y,
                    -vp.localAngularVel.z * stopFlipFactor.z), ForceMode.Acceleration);
        }

        // Apply forces to align vehicle with normal of ground surface that it will land on
        private void ApplyRotationCorrection() {
            var actualForwardDot = vp.forwardDot;
            var actualRightDot = vp.rightDot;
            var actualUpDot = vp.upDot;

            if (groundCheckDistance > 0) {
                if (Physics.Raycast(tr.position, (-GlobalControl.worldUpDir + rb.velocity).normalized, out var groundHit,
                    groundCheckDistance, GlobalControl.groundMaskStatic))
            ...
            rb.AddRelativeTorque(new Vector3(
                vp.pitchInput * flipPower.x == 0
                    ? actualForwardDot * (1 - Mathf.Abs(actualRightDot)) * rotationCorrection.x -
                      vp.localAngularVel.x * Mathf.Pow(actualUpDot, 2) * 10
                    : 0,
                vp.yawInput * flipPower.y == 0 && vp.sqrVelMag > 10
                    ? Vector3.Dot(vp.forwardDir, velDir * Vector3.right) * Mathf.Abs(actualUpDot) *
                      rotationCorrection.y - vp.localAngularVel.y * Mathf.Pow(actualUpDot, 2) * 10
                    : 0,
                vp.rollInput * flipPower.z == 0
                    ? -actualRightDot * (1 - Mathf.Abs(actualForwardDot)) * rotationCorrection.z -
                      vp.localAngularVel.z * Mathf.Pow(actualUpDot, 2) * 10
                    : 0
        ...
        private void Dive() {
            rb.AddTorque(
                velDir * Vector3.left * Mathf.Clamp01(vp.velMag * 0.01f) * Mathf.Clamp01(vp.upDot) * diveFactor,
                ForceMode.Acceleration);
        }
    }
}