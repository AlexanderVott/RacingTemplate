...
namespace RVP {
    ...
    public class TerrainSurface : MonoBehaviour {
        ...
        [System.NonSerialized] public float[] frictions;
        ...
        public int GetDominantSurfaceTypeAtPoint(Vector3 pos) {
            ...
            var coord = new Vector2(Mathf.Clamp01((pos.z - tr.position.z) / terDat.size.z),
                Mathf.Clamp01((pos.x - tr.position.x) / terDat.size.x));

            float maxVal = 0;
            var maxIndex = 0;
            float curVal = 0;

            for (var i = 0; i < terrainAlphamap.GetLength(2); i++) {
                curVal = terrainAlphamap[Mathf.FloorToInt(coord.x * (terDat.alphamapWidth - 1)),
                    Mathf.FloorToInt(coord.y * (terDat.alphamapHeight - 1)), i];
