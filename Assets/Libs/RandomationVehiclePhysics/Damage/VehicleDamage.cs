...
namespace RVP {
    ...
    public class VehicleDamage : MonoBehaviour {
        ...
        private void Start() {
            ...
            for (var i = 0; i < displaceParts.Length; i++)
            ...
            if (hitTime == 0 && col.relativeVelocity.sqrMagnitude * damageFactor > 1 && strength < 1) {
                ...
                foreach (var curCol in col.contacts) {
                    if (tr.InverseTransformPoint(curCol.point).y > collisionIgnoreHeight &&
                        ...
                        DamageApplication(curCol.point, col.relativeVelocity, maxCollisionMagnitude, curCol.normal,
                            curCol, true);
                    }
        ...
        public void ApplyDamage(Vector3[] damagePoints, Vector3 damageForce) {
            foreach (var curDamagePoint in damagePoints)
                DamageApplication(curDamagePoint, damageForce, Mathf.Infinity, damageForce.normalized, nullContact,
                    false);
        ...
        public void ApplyDamage(Vector3[] damagePoints, Vector3 damageForce, float damageForceLimit) {
            foreach (var curDamagePoint in damagePoints)
                DamageApplication(curDamagePoint, damageForce, damageForceLimit, damageForce.normalized, nullContact,
                    false);
        ...
        private void DamageApplication(Vector3 damagePoint, Vector3 damageForce, float damageForceLimit,
                                       Vector3 surfaceNormal, ContactPoint colPoint, bool useContactPoint) {
            var colMag = Mathf.Min(damageForce.magnitude, maxCollisionMagnitude) * (1 - strength) *
            ...
            for (var i = 0; i < damageParts.Length; i++) {
                ...
                damagePartFactor = colMag * surfaceDot * massFactor * Mathf.Min(clampedColMag * 0.01f,
                                       ...
                                       Mathf.Pow(Vector3.Distance(curDamagePart.position, damagePoint), clampedColMag));
            ...
            for (var i = 0; i < deformMeshes.Length; i++) {
                ...
                if (translation.sqrMagnitude > 0 && strength < 1)
                    for (var j = 0; j < meshVertices[i].verts.Length; j++) {
                        ...
                        if (distClamp > 0.001f) {
                            ...
                            if (seamKeeper == null || seamlessDeform) {
                                vertProjection = seamlessDeform
                                                     ? Vector3.zero
                                                     : Vector3.Project(normalizedVel, meshVertices[i].verts[j]);
                                meshVertices[i].verts[j] +=
                                    (clampedTranslation - vertProjection *
                                     (usePerlinNoise
                                          ? 1 + Mathf.PerlinNoise(meshVertices[i].verts[j].x * 100,
                                                meshVertices[i].verts[j].y * 100)
                                          : 1)) * surfaceDot * Mathf.Min(clampedColMag * 0.01f, distClamp) * massFactor;
                            }
                            else {
                                seamLocalPoint =
                                    seamKeeper.InverseTransformPoint(
                                        curDamageMesh.transform.TransformPoint(meshVertices[i].verts[j]));
                                meshVertices[i].verts[j] +=
            ...
            for (var i = 0; i < deformColliders.Length; i++) {
                ...
                if (translation.sqrMagnitude > 0 && strength < 1)
                    for (var j = 0; j < colVertices[i].verts.Length; j++) {
                        vertDist = (colVertices[i].verts[j] - localPos).sqrMagnitude;
                        distClamp = clampedColMag * 0.001f / Mathf.Pow(vertDist, clampedColMag * 0.5f);

                        if (distClamp > 0.001f) {
                            damagedCols[i] = true;
                            colVertices[i].verts[j] += clampedTranslation * surfaceDot *
            ...
            for (var i = 0; i < displaceParts.Length; i++) {
                ...
                if (translation.sqrMagnitude > 0 && strength < 1) {
                    ...
                    if (distClamp > 0.001f) {
                        curDisplacePart.position += clampedTranslation * surfaceDot *
                                                    Mathf.Min(clampedColMag * 0.01f, distClamp) * massFactor;
                        ...
                        // Maybe the parent of this part is what actually detaches, useful for displacing compound colliders that represent single detachable objects
                            ...
                            if (!detachedPart.detached) {
                                ...
                                detachedPart.displacedAnchor +=
                                    curDisplacePart.parent.InverseTransformDirection(
                                        clampedTranslation * surfaceDot * Mathf.Min(clampedColMag * 0.01f, distClamp) *
                                        massFactor);
                            }
                        }
                                    ...
                                    if (curPart.connectObj && !curPart.isHub && !curPart.solidAxle)
                                        if (!curPart.connectObj.GetComponent<SuspensionPart>())
                                            curPart.connectPoint +=
                                                curPart.connectObj.InverseTransformDirection(
                                                    clampedTranslation * surfaceDot *
                                                    Mathf.Min(clampedColMag * 0.01f, distClamp) * massFactor);
                                }
                            }
        ...
        public void Repair() {
            ...
            for (var i = 0; i < deformMeshes.Length; i++) {
                ...
                // Fix shattered parts
                var fixedShatter = deformMeshes[i].GetComponent<ShatterPart>();
                if (fixedShatter) {
            ...
            // Fix hover wheels
            foreach (var curHoverWheel in vp.hoverWheels)
                curHoverWheel.Reattach();
        }

        // Draw collisionIgnoreHeight gizmos
        private void OnDrawGizmosSelected() {
            var startPoint = transform.TransformPoint(Vector3.up * collisionIgnoreHeight);
        ...
        private void OnDestroy() {
            foreach (var curPart in displaceParts) {
                if (curPart) {
