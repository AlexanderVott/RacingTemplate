...
{
	public static class ToolsCacheExtensions
	{
	...
	public static class ImmutableExtensions
	{
		...
		private static int CountInternal<T>(this T[] array, Predicate<T> predicate)
		{
			...
			if (ToolsCacheExtensions.PreCache.Length < array.Length)
			{
				Debug.LogError("ArrayExtension: PreCache depleted, re-allocate!");
				ToolsCacheExtensions.PreCache = ToolsCacheExtensions.PreCache.Resize(array.Length);
			}

			var count = 0;
			for (int i = 0; i < array.Length; ++i)
			{
				ToolsCacheExtensions.PreCache[i] = predicate(array[i]);
				if (ToolsCacheExtensions.PreCache[i])
				{
					++count;
				}
			}
		...
		private static int CountInternal<T>(this T[] array, Func<T, int, bool> predicate)
		{
			...
			for (int i = 0; i < array.Length; ++i)
			{
				ToolsCacheExtensions.PreCache[i] = predicate(array[i], i);
				if (ToolsCacheExtensions.PreCache[i])
				{
					++count;
				}
		...
		public static T SelectRandom<T>(this T[] array, Predicate<T> predicate)
		{
			...
			if (count > 0)
			{
				if (ToolsCacheExtensions.PreCacheInt.Length < count)
				{
					Debug.LogError("ArrayExtension: PreCacheInt depleted, re-allocate!");
					ToolsCacheExtensions.PreCacheInt = ToolsCacheExtensions.PreCacheInt.Resize(array.Length);
				}
				...
				for (int i = 0, j = 0; i < array.Length; ++i)
				{
					if (ToolsCacheExtensions.PreCache[i])
					{
						ToolsCacheExtensions.PreCacheInt[j] = i;
						++j;
					}
				}

				return array[ToolsCacheExtensions.PreCacheInt[UnityEngine.Random.Range(0, count)]];
			}
		...
		public static int SelectRandomIndex<T>(this T[] array, Predicate<T> predicate)
		{
			...
			if (count > 0)
			{
				...
				return ToolsCacheExtensions.PreCacheInt[UnityEngine.Random.Range(0, count)];
			}
